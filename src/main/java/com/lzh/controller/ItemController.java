...
public class ItemController {
     ...
     * */
   /* @RequestMapping("/add")
    public String add(MultipartFile pic,
                      HttpServletRequest request
                      //Item item

    ) throws IOException, InvocationTargetException, IllegalAccessException {//MultipartFile 的参数必须是form对应文件输入框的name的值pic
        ...
        if (pic != null && pic.getSize() > 0) {
            //System.out.println("图片的大小" + picFile.getSize());
            //1.给图片重新取名字
            //1.1获取图片的原名
            ...
            //3.封装一个访问路径
            String picPath = "http://localhost/static/images/" + newsName;

            String[] pics = {picPath};

            //给itemMap中的pic重新赋值(将图片名改为路劲)
            itemMap.put("pic", pics);

            //4.BeanUtils通过将from表单数据映射POJO类
...
*/
     ...
     * 为了修改商品页面回显查询商品
     **/
    /*@RequestMapping("/updatetofindbyid")
    public String updateToFindById(Long id, Model model) {
    ...
    public String update(MultipartFile pic,
                         HttpServletRequest request
        ...
        //修改失败(转发)
        request.setAttribute("updateInfo","编号为:"+item.getId()+"的商品修改失败!!!");
    ...
    public String delete(Long id, Model model){
        ...
        boolean b = itemService.deleteItemById(id);
        ...
        model.addAttribute("deleteInfo","编号为:"+id+"的商品修改失败!!!");
